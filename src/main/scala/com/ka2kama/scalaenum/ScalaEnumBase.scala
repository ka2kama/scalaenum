...
import enumeratum.values.{ByteEnum, CharEnum, IntEnum, LongEnum, ShortEnum, StringEnum, ValueEnum, ValueEnumEntry}
...
/** [[enumeratum.values.ValueEnum]]統一用の基底クラス。各メンバーはvalueという名前の特定の値を持つ
  ...
  * @tparam EntryType 列挙メンバーの型
  */
sealed trait ScalaValueEnum[ValueType, EntryType <: ValueScalaEnumEntry[ValueType] with ValueEnumEntry[ValueType]]
  ...
  protected lazy final val namesToValuesMap: Map[String, EntryType] = values.map(v => v.name -> v).toMap
  ...
  final def withNameInsensitive(name: String): EntryType =
    withNameInsensitiveOption(name).getOrElse(throw new NoSuchElementException(buildNameNotFoundMessage(name)))
  final def withNameInsensitiveOption(name: String): Option[EntryType] = lowerCaseNamesToValuesMap.get(name.toLowerCase)

  final def withNameUppercaseOnly(name: String): EntryType =
    withNameUppercaseOnlyOption(name).getOrElse(throw new NoSuchElementException(buildNameNotFoundMessage(name)))
  final def withNameUppercaseOnlyOption(name: String): Option[EntryType] = upperCaseNameValuesToMap.get(name)

  final def withNameLowercaseOnly(name: String): EntryType =
    withNameLowercaseOnlyOption(name).getOrElse(throw new NoSuchElementException(buildNameNotFoundMessage(name)))
  final def withNameLowercaseOnlyOption(name: String): Option[EntryType] = lowerCaseNamesToValuesMap.get(name)
...
trait ScalaEnum[A <: ScalaEnumEntry] extends enumeratum.Enum[A] with ScalaEnumBase[A] {
  ...
  override final def withNameInsensitiveOption(name: String): Option[A]   = super.withNameInsensitiveOption(name)
  override final def withNameUppercaseOnlyOption(name: String): Option[A] = super.withNameUppercaseOnlyOption(name)
  override final def withNameLowercaseOnlyOption(name: String): Option[A] = super.withNameLowercaseOnlyOption(name)
  override final def indexOf(member: A): Int                              = super.indexOf(member)
}

/** valueとしてInt値を持つ列挙型用のtrait */
trait IntScalaEnum[A <: IntScalaEnumEntry] extends IntEnum[A] with ScalaValueEnum[Int, A]

/** valueとしてInt値を持つ列挙型用のtrait */
trait LongScalaEnum[A <: LongScalaEnumEntry] extends LongEnum[A] with ScalaValueEnum[Long, A]

/** valueとしてLong値を持つ列挙型用のtrait */
trait ShortScalaEnum[A <: ShortScalaEnumEntry] extends ShortEnum[A] with ScalaValueEnum[Short, A]

/** valueとしてString値を持つ列挙型用のtrait */
trait StringScalaEnum[A <: StringScalaEnumEntry] extends StringEnum[A] with ScalaValueEnum[String, A]

/** valueとしてByte値を持つ列挙型用のtrait */
trait ByteScalaEnum[A <: ByteScalaEnumEntry] extends ByteEnum[A] with ScalaValueEnum[Byte, A]

/** valueとしてChar値を持つ列挙型用のtrait */
trait CharScalaEnum[A <: CharScalaEnumEntry] extends CharEnum[A] with ScalaValueEnum[Char, A]
